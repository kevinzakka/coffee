"""This module contains a URDF editor class.
...
class UrdfEditor(object):
    ...
    def convertLinkFromMultiBody(self, bodyUid, linkIndex, urdfLink, physicsClientId):
        ...
        for v in visualShapes:
            if v[1] == linkIndex:
                ...
                name = "mat_{}_{}_{}".format(linkIndex, matIndex, uuid.uuid4())
    ...
    def writeVisualShape(self, file, urdfVisual, precision=5):
        ...
        if urdfVisual.geom_type == p.GEOM_CAPSULE:
            str = (
                '\t\t\t\t<capsule length="{:.{prec}f}" radius="{:.{prec}f}"/>\n'.format(
                    urdfVisual.geom_length, urdfVisual.geom_radius, prec=precision
                )
            )
            file.write(str)
    ...
    def joinUrdf(
        ...
        self.urdfLinks[
            childLinkIndex
        ].urdf_inertial.origin_rpy = p.getEulerFromQuaternion(orn)
        # all visual
    ...
    def createMultiBody(
        self, basePosition=[0, 0, 0], baseOrientation=[0, 0, 0, 1], physicsClientId=0
    ):
        ...
        for v in base.urdf_collision_shapes:
            ...
            baseHalfExtentsArray.append(
                [
                    0.5 * v.geom_extents[0],
                    0.5 * v.geom_extents[1],
